...
const COMMAND_DOCS: Record<string, CommandDoc> = {
  toggle: {
    name: 'toggle', category: 'DOM Manipulation',
  ...
  add: {
    name: 'add', category: 'DOM Manipulation',
  ...
  remove: {
    name: 'remove', category: 'DOM Manipulation',
  ...
  show: {
    name: 'show', category: 'DOM Manipulation',
  ...
  hide: {
    name: 'hide', category: 'DOM Manipulation',
  ...
  put: {
    name: 'put', category: 'DOM Manipulation',
  ...
  fetch: {
    ...
    examples: ['fetch /api/data as json', 'fetch /page as html', 'fetch /api then put it into #output'],
  },
  call: {
    name: 'call', category: 'Utility',
  ...
  transition: {
    name: 'transition', category: 'Animation',
  ...
  tell: {
    name: 'tell', category: 'Control Flow',
  ...
  'if': {
    ...
    examples: ['if me matches .active hide me else show me end', 'if #input.value is empty add .error end'],
  },
  repeat: {
    name: 'repeat', category: 'Control Flow',
  ...
  'for': {
    name: 'for', category: 'Control Flow',
...
const EXPRESSION_DOCS: Record<string, ExpressionDoc> = {
  me: { name: 'me', category: 'references', evaluatesTo: 'Element', description: 'Reference to the current element (the element with the _ attribute)', examples: ['toggle .active on me', 'put "Hello" into me'] },
  you: { name: 'you', category: 'references', evaluatesTo: 'Element', description: 'Reference to the element that triggered the event', examples: ['add .selected to you'] },
  it: { name: 'it', category: 'references', evaluatesTo: 'Any', description: 'Reference to the result of the previous command', examples: ['fetch /api then put it into #output'] },
  result: { name: 'result', category: 'references', evaluatesTo: 'Any', description: 'Alias for "it"', examples: ['fetch /api then put result into #output'] },
  ...
  closest: { name: 'closest', category: 'references', evaluatesTo: 'Element', description: 'Find the nearest ancestor matching a selector', examples: ['toggle .open on closest .accordion-item'] },
  parent: { name: 'parent', category: 'references', evaluatesTo: 'Element', description: 'Direct parent element', examples: ['add .highlight to parent'] },
  first: { name: 'first', category: 'positional', evaluatesTo: 'Any', description: 'First item from a collection', examples: ['first of .items', 'first <li/>'] },
  last: { name: 'last', category: 'positional', evaluatesTo: 'Any', description: 'Last item from a collection', examples: ['last of .items', 'last <li/>'] },
  next: { name: 'next', category: 'positional', evaluatesTo: 'Element', description: 'Next sibling matching a selector', examples: ['toggle .open on next .panel'] },
  previous: { name: 'previous', category: 'positional', evaluatesTo: 'Element', description: 'Previous sibling matching a selector', examples: ['previous <li/>'] },
  possessive: { name: 'possessive', category: 'properties', evaluatesTo: 'Any', description: "Access element properties using 's or my/its/your", examples: ["#input's value", 'my textContent', 'its length'] },
  'attribute-ref': { name: 'attribute-ref', category: 'properties', evaluatesTo: 'String', description: 'Reference to a DOM attribute using @ prefix', examples: ['toggle @disabled', 'set @data-id to "123"'] },
  'style-ref': { name: 'style-ref', category: 'properties', evaluatesTo: 'String', description: 'Reference to a CSS style property using * prefix', examples: ['set *background-color to "red"'] },
  ...
  matches: { name: 'matches', category: 'logical', evaluatesTo: 'Boolean', description: 'Check if element matches a CSS selector', examples: ['if me matches .disabled'] },
  contains: { name: 'contains', category: 'logical', evaluatesTo: 'Boolean', description: 'Check if string/array contains a value', examples: ['if "hello" contains "ell"'] },
  exists: { name: 'exists', category: 'logical', evaluatesTo: 'Boolean', description: 'Check if a value/element exists', examples: ['if #modal exists'] },
  'is-empty': { name: 'is-empty', category: 'logical', evaluatesTo: 'Boolean', description: 'Check if value is empty', examples: ['if #input.value is empty'] },
  has: { name: 'has', category: 'logical', evaluatesTo: 'Boolean', description: 'Check if element has a class or attribute', examples: ['if me has .active'] },
};
  ...
  try {
    switch (name) {
      case 'get_command_docs': {
        ...
        if (!command) {
          return {
            content: [{ type: 'text', text: JSON.stringify({ error: 'Missing required parameter: command' }, null, 2) }],
            isError: true,
          };
        }
      ...
      case 'get_expression_docs': {
        ...
        if (!expression) {
          return {
            content: [{ type: 'text', text: JSON.stringify({ error: 'Missing required parameter: expression' }, null, 2) }],
            isError: true,
          };
        }
      ...
      case 'search_language_elements': {
        ...
        if (!query) {
          return {
            content: [{ type: 'text', text: JSON.stringify({ error: 'Missing required parameter: query' }, null, 2) }],
            isError: true,
          };
        }
        ...
        for (const [, doc] of Object.entries(COMMAND_DOCS)) {
          if (doc.name.includes(query) || doc.description.toLowerCase().includes(query) || doc.category.toLowerCase().includes(query)) {
        ...
        for (const [, doc] of Object.entries(EXPRESSION_DOCS)) {
          if (doc.name.includes(query) || doc.description.toLowerCase().includes(query) || doc.category.includes(query)) {
        ...
        return {
          content: [
            {
              type: 'text',
              text: JSON.stringify({ query, results: results.slice(0, limit), total: results.length }, null, 2),
            },
          ],
        };
      }
