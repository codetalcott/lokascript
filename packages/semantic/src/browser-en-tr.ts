/**
 * English + Turkish Browser Bundle Entry Point
 *
 * Minimal bundle supporting English + Turkish.
 * Estimated size: ~90 KB (~31 KB gzip)
 *
 * @example
 * ```html
 * <script src="hyperfixi-semantic.en-tr.global.js"></script>
 * <script>
 *   const ast = LokaScriptSemanticEnTr.parse('toggle .active', 'en');
 * </script>
 * ```
 */

// =============================================================================
// Register Languages: en, tr
// =============================================================================
...
function validateLanguage(language: string): void {
  if (!SUPPORTED_LANGUAGES.includes(language as (typeof SUPPORTED_LANGUAGES)[number])) {
    throw new Error(`Language not supported in this bundle: ${language}. ` + `Supported: en, tr`);
  }
}
...
export { turkishProfile } from './generators/language-profiles';
