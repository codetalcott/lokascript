/**
 * Chinese Browser Bundle Entry Point
 *
 * Minimal single-language bundle.
 * Estimated size: ~65 KB (~23 KB gzip)
 *
 * @example
 * ```html
 * <script src="hyperfixi-semantic.zh.global.js"></script>
 * <script>
 *   const ast = LokaScriptSemanticZh.parse('toggle .active', 'zh');
 * </script>
 * ```
 */

// =============================================================================
// Register Languages: zh
// =============================================================================
...
function validateLanguage(language: string): void {
  if (language !== 'zh') {
    throw new Error(`Language not supported in this bundle: ${language}. ` + `Supported: zh`);
  }
}
...
export { chineseProfile } from './generators/language-profiles';
