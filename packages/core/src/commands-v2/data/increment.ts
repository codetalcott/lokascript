/**
 * IncrementCommand V2 - Standalone Implementation (Zero V1 Dependencies)
 *
 * Increases the value of a variable or element property by a specified amount with full support for:
 * - Variables: increment counter, increment global score by 10
 * - Element properties: increment me.scrollTop by 100
 * - Scoped variables: increment :global counter, increment :local counter
 * - Default increment: 1 (if no 'by' amount specified)
 *
...
/**
 * Standalone IncrementCommand V2 - Zero V1 dependencies
 *
 * This implementation completely rewrites increment command logic with:
...
export class IncrementCommand {
  ...
  /**
   * Convert any value to a number for increment operations
   *
   * Handles:
   ...
   * @param incrementBy - Amount to add
   * @returns New value
   */
   ...
   *
   * Increment command syntax: "increment <target> [by <amount>]"
   * Also supports: "increment global <target> by <amount>"
   *
   * Examples:
   * - increment counter
