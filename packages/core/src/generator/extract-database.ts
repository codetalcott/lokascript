...
export class LSPDatabaseExtractor {
  ...
  constructor(dbPath: string = LOCAL_DB_PATH) {
    ...
    if (!existsSync(dbPath)) {
      throw new Error(
        `Database not found at ${dbPath}. Please ensure hyperscript-lsp project is available.`
      );
    }
...
export type HyperscriptCommand = ${commands
      .map(cmd => `${cmd.name.charAt(0).toUpperCase() + cmd.name.slice(1)}Command`)
      .join(' | ')};
`;
  }
  ...
  generateTestCases(): string {
    ...
    const testsByType = examples.reduce(
      (acc, example) => {
    ...
    const testSuites = Object.entries(testsByType)
      .map(([elementType, elements]) => {
        const elementTests = Object.entries(elements)
          .map(([elementName, elementExamples]) => {
            const tests = elementExamples
              .map(
                (example, index) => `
...
`;
  ...
  async extract(): Promise<void> {
    ...
    console.log(
      `Found ${commands.length} commands, ${expressions.length} expressions, ${features.length} features, ${examples.length} examples`
    );
