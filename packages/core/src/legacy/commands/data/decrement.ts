...
export class DecrementCommand implements CommandImplementation {
  ...
  async execute(context: ExecutionContext, ...args: any[]): Promise<any> {
    ...
    if (args[0] === null || args[0] === undefined) {
      throw new Error('Cannot decrement null or undefined target');
    }
    ...
    const newValue = this.performDecrement(currentValue, parsedArgs.amount);
  ...
  validate(args: any[]): string | null {
    ...
    if (args.length >= 2) {
      ...
      if (args[1] !== 'by') {
        return 'Decrement command requires "by" keyword when specifying amount';
      }
      ...
      if (typeof amount !== 'number' && (typeof amount !== 'string' || isNaN(parseFloat(amount)))) {
        return 'Decrement command requires numeric amount after "by" keyword';
      }
    }
  ...
  private parseArguments(args: any[]): { target: any; property?: string; scope?: string; amount: number } {
    ...
    let amount = 1; // Default decrement
    
    // Handle different argument patterns
    if (args[0] === 'global') {
      // Pattern: decrement global variableName [by amount]
      scope = 'global';
      target = args[1];
      ...
      // Pattern: decrement element/object property/attribute [by amount]
      ...
      // Pattern: decrement variableName [by amount]
      target = args[0];
