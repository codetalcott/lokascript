...
describe('Repeat Command', () => {
  ...
  describe('Command Properties', () => {
    it('should have correct metadata', () => {
      ...
      expect(command.syntax).toBe('repeat for <identifier> in <expression> [index <identifier>] { <command> } end');
      expect(command.description).toBe('The repeat command provides iteration in the hyperscript language.');
    });
  });
  ...
  describe('Basic For Loop - repeat for ... in', () => {
    ...
    it('should set iterator variable in local scope', async () => {
      ...
      let capturedLocals: Map<string, any>[] = [];
