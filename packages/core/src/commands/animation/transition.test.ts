...
describe('Transition Command', () => {
  ...
  describe('Basic Transitions', () => {
    ...
    it('should transition multiple properties', async () => {
      await transitionCommand.execute(context, 
        'opacity', 'to', '0.5',
        'left', 'to', '100px'
      );
  ...
  describe('Event Integration', () => {
    it('should wait for transition to complete before resolving', async () => {
      ...
      const promise = transitionCommand.execute(context, 
        'opacity', 'to', '0.5', 'over', '200ms'
      );
  ...
  describe('Error Handling', () => {
    it('should handle invalid CSS property names gracefully', async () => {
      await expect(transitionCommand.execute(context, 'invalid-property', 'to', 'value'))
    ...
    it('should handle element not found', async () => {
      await expect(transitionCommand.execute(context, '#non-existent', 'opacity', 'to', '0.5'))
  ...
  describe('Validation', () => {
    ...
    it('should reject invalid timing syntax', () => {
      expect(transitionCommand.validate(['opacity', 'to', '0.5', 'over'])).toContain('Duration required after "over"');
      expect(transitionCommand.validate(['opacity', 'to', '0.5', 'with'])).toContain('Timing function required after "with"');
      expect(transitionCommand.validate(['opacity', 'to', '0.5', 'delay'])).toContain('Delay value required after "delay"');
    });
  });
});