...
describe('Default Command', () => {
  ...
  describe('Property Defaults', () => {
    ...
    it('should handle custom DOM properties', async () => {
      ...
      const result = await defaultCommand.execute(context, 'me.customProp', 'to', 'default content');
  ...
  describe('Error Handling', () => {
    it('should throw error for missing arguments', async () => {
      await expect(defaultCommand.execute(context))
        .rejects.toThrow('Default command requires at least 3 arguments');
    });

    it('should throw error for missing "to" keyword', async () => {
      await expect(defaultCommand.execute(context, 'x', 'not-to', 'value'))
        .rejects.toThrow('Default command requires "to" keyword');
    });

    it('should throw error for invalid context reference', async () => {
      context.me = null;
      
      await expect(defaultCommand.execute(context, 'my foo', 'to', 'value'))
        .rejects.toThrow('Context reference \'my\' is not available');
    });
  });
