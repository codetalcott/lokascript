...
describe('Tokens', () => {
  ...
  it('should require tokens', () => {
    ...
    expect(() => tokens.requireToken('foo'))
      .toThrow("Expected 'foo' but found 'world'");
  });
  ...
  it('should raise errors with position information', () => {
    expect(() => tokens.raiseError('test error'))
      .toThrow('Parse error at line 1, column 1: test error');
  });
});
