  ...
  describe('StyleRef Expressions', () => {
    ...
    describe('basic styleRef expression', () => {
      ...
      it('should get computed style properties with computed- prefix', async () => {
        const computedColorResult = await referenceExpressions.styleRef.evaluate(context, 'computed-color');
        expect(computedColorResult).toBe('red'); // Happy-DOM returns inline value as is

        const computedTextAlignResult = await referenceExpressions.styleRef.evaluate(context, 'computed-text-align');
        expect(computedTextAlignResult).toBe('center');

        const computedWidthResult = await referenceExpressions.styleRef.evaluate(context, 'computed-width');
        expect(computedWidthResult).toBe('10px');
      });

      it('should return empty string for unset computed properties', async () => {
        const computedHeightResult = await referenceExpressions.styleRef.evaluate(context, 'computed-height');
        expect(computedHeightResult).toBe(''); // Happy-DOM returns empty for unset properties

        const computedBadPropResult = await referenceExpressions.styleRef.evaluate(context, 'computed-bad-prop');
        expect(computedBadPropResult).toBe('');
      });

      it('should work with explicit element parameter', async () => {
        const explicitResult = await referenceExpressions.styleRef.evaluate(context, 'color', testElement);
      ...
      it('should validate arguments correctly', () => {
        ...
        expect(referenceExpressions.styleRef.validate!(['color', testElement, 'extra'])).toContain('requires 1-2 arguments');
        expect(referenceExpressions.styleRef.validate!([123])).toContain('must be a string');
        expect(referenceExpressions.styleRef.validate!(['color', 'not-element'])).toContain('must be an HTMLElement');
      });

      it('should throw error for non-string property', async () => {
        await expect(referenceExpressions.styleRef.evaluate(context, 123 as any))
    ...
    describe('possessive styleRef expression', () => {
      it('should get style properties with possessive syntax', async () => {
        const myColorResult = await referenceExpressions.possessiveStyleRef.evaluate(context, 'my', 'color');
        expect(myColorResult).toBe('red');

        const myTextAlignResult = await referenceExpressions.possessiveStyleRef.evaluate(context, 'my', 'text-align');
        expect(myTextAlignResult).toBe('center');

        const myWidthResult = await referenceExpressions.possessiveStyleRef.evaluate(context, 'my', 'width');
        expect(myWidthResult).toBe('10px');
      });

      it('should work with its possessor using result context', async () => {
        context.result = testElement;
        const itsHeightResult = await referenceExpressions.possessiveStyleRef.evaluate(context, 'its', 'height');
        expect(itsHeightResult).toBeUndefined();

        const itsBadPropResult = await referenceExpressions.possessiveStyleRef.evaluate(context, 'its', 'bad-prop');
        expect(itsBadPropResult).toBeUndefined();
      });

      it('should get computed style properties with possessive syntax', async () => {
        const myComputedColorResult = await referenceExpressions.possessiveStyleRef.evaluate(context, 'my', 'computed-color');
        expect(myComputedColorResult).toBe('red'); // Happy-DOM returns inline value as is

        const myComputedTextAlignResult = await referenceExpressions.possessiveStyleRef.evaluate(context, 'my', 'computed-text-align');
        expect(myComputedTextAlignResult).toBe('center');

        const myComputedWidthResult = await referenceExpressions.possessiveStyleRef.evaluate(context, 'my', 'computed-width');
        expect(myComputedWidthResult).toBe('10px');
      });

      it('should handle computed style for its possessor', async () => {
        context.result = testElement;
        const itsComputedHeightResult = await referenceExpressions.possessiveStyleRef.evaluate(context, 'its', 'computed-height');
        expect(itsComputedHeightResult).toBe(''); // Happy-DOM returns empty for unset properties

        const itsComputedBadPropResult = await referenceExpressions.possessiveStyleRef.evaluate(context, 'its', 'computed-bad-prop');
        expect(itsComputedBadPropResult).toBe('');
      });

      it('should return undefined for invalid possessor', async () => {
        const result = await referenceExpressions.possessiveStyleRef.evaluate(context, 'invalid', 'color');
        expect(result).toBeUndefined();
      });

      it('should validate arguments correctly', () => {
        expect(referenceExpressions.possessiveStyleRef.validate!(['my', 'color'])).toBeNull();
        expect(referenceExpressions.possessiveStyleRef.validate!(['its', 'computed-width'])).toBeNull();
        expect(referenceExpressions.possessiveStyleRef.validate!([])).toContain('requires exactly 2 arguments');
        expect(referenceExpressions.possessiveStyleRef.validate!(['my'])).toContain('requires exactly 2 arguments');
        expect(referenceExpressions.possessiveStyleRef.validate!(['my', 'color', 'extra'])).toContain('requires exactly 2 arguments');
        expect(referenceExpressions.possessiveStyleRef.validate!([123, 'color'])).toContain('possessor must be a string');
        expect(referenceExpressions.possessiveStyleRef.validate!(['my', 123])).toContain('property must be a string');
      });

      it('should throw error for non-string arguments', async () => {
        await expect(referenceExpressions.possessiveStyleRef.evaluate(context, 123 as any, 'color'))
          .rejects.toThrow('Possessive styleRef requires possessor and property strings');
        await expect(referenceExpressions.possessiveStyleRef.evaluate(context, 'my', 123 as any))
          .rejects.toThrow('Possessive styleRef requires possessor and property strings');
      });
    });
    ...
    describe('of styleRef expression', () => {
      it('should get style properties with of syntax', async () => {
        const colorOfMeResult = await referenceExpressions.ofStyleRef.evaluate(context, 'color', 'me');
        expect(colorOfMeResult).toBe('red');

        const textAlignOfMeResult = await referenceExpressions.ofStyleRef.evaluate(context, 'text-align', 'me');
        expect(textAlignOfMeResult).toBe('center');

        const widthOfMeResult = await referenceExpressions.ofStyleRef.evaluate(context, 'width', 'me');
        expect(widthOfMeResult).toBe('10px');
      });

      it('should work with it reference using result context', async () => {
        context.result = testElement;
        const heightOfItResult = await referenceExpressions.ofStyleRef.evaluate(context, 'height', 'it');
        expect(heightOfItResult).toBeUndefined();

        const badPropOfItResult = await referenceExpressions.ofStyleRef.evaluate(context, 'bad-prop', 'it');
        expect(badPropOfItResult).toBeUndefined();
      });

      it('should get computed style properties with of syntax', async () => {
        const computedColorOfMeResult = await referenceExpressions.ofStyleRef.evaluate(context, 'computed-color', 'me');
        expect(computedColorOfMeResult).toBe('red'); // Happy-DOM returns inline value as is

        const computedTextAlignOfMeResult = await referenceExpressions.ofStyleRef.evaluate(context, 'computed-text-align', 'me');
        expect(computedTextAlignOfMeResult).toBe('center');

        const computedWidthOfMeResult = await referenceExpressions.ofStyleRef.evaluate(context, 'computed-width', 'me');
        expect(computedWidthOfMeResult).toBe('10px');
      });

      it('should handle computed style for it reference', async () => {
        context.result = testElement;
        const computedHeightOfItResult = await referenceExpressions.ofStyleRef.evaluate(context, 'computed-height', 'it');
        expect(computedHeightOfItResult).toBe(''); // Happy-DOM returns empty for unset properties

        const computedBadPropOfItResult = await referenceExpressions.ofStyleRef.evaluate(context, 'computed-bad-prop', 'it');
      ...
      it('should validate arguments correctly', () => {
        ...
        expect(referenceExpressions.ofStyleRef.validate!([])).toContain('requires exactly 2 arguments');
        expect(referenceExpressions.ofStyleRef.validate!(['color'])).toContain('requires exactly 2 arguments');
        expect(referenceExpressions.ofStyleRef.validate!(['color', 'me', 'extra'])).toContain('requires exactly 2 arguments');
        expect(referenceExpressions.ofStyleRef.validate!([123, 'me'])).toContain('property must be a string');
        expect(referenceExpressions.ofStyleRef.validate!(['color', 123])).toContain('reference must be a string');
      });

      it('should throw error for non-string arguments', async () => {
        await expect(referenceExpressions.ofStyleRef.evaluate(context, 123 as any, 'me'))
          .rejects.toThrow('Of styleRef requires property and reference strings');
        await expect(referenceExpressions.ofStyleRef.evaluate(context, 'color', 123 as any))
          .rejects.toThrow('Of styleRef requires property and reference strings');
      });
    });
  });
